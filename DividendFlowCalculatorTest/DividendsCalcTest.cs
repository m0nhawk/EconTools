...
namespace GivenAData
{
	...
	public class OnCalculation
	{
		...
		public void Init ()
		{
			...
			shareholdings = Matrix<double>.Build.DenseOfArray (new double[,] { { 0, 0.1, 0.2 },
				{ 0, 0, 0.15 },
				{ 0.12, 0.12, 0 }
			});
			...
			ownership = Matrix<double>.Build.DenseOfArray (new double[,] { { 0.1129679439, 0.0142647149, 0.0247332029 },
			...
			penultimateExit = Matrix<double>.Build.DenseOfArray (new double[,] { { 110, 0.2070696487, 2.7609286707 },
				{ 0, 0, 3.9531478695 },
		...
		public void CorrectDynamicDidivend ()
		{
			...
			dynamicDividend = DividendFlowCalculator.DividendFlowCalculator.dynamicDividendTable (d0, f);
		}

		[Test]
		public void CorrectOwnershipTable ()
		{
			var res = DividendFlowCalculator.DividendFlowCalculator.ownershipTable (f);
		...
		{
			var res = DividendFlowCalculator.DividendFlowCalculator.penultimateExitTable (dynamicDividend, shareholdings, outside, remainder);
...
namespace GivenAFile
{
	...
	public class OnLoad
	{
		...
		public void Success ()
		{
			...
			readShareholdings = vals.Item1;
			n = readShareholdings.ColumnCount;
			Assert.AreEqual (n, length);

			readOutside = vals.Item2;
			readRemainder = vals.Item3;
			readDividends = vals.Item4;
